...
// === DEBUG: H1 Font Size Checker ===
document.addEventListener('DOMContentLoaded', function() {
  // Wait a bit for styles to load
  setTimeout(function() {
    console.log('=== H1 FONT SIZE DEBUG ===');
    ...
    h1Elements.forEach((h1, index) => {
      ...
      const fontSizeRem = fontSizePx / 16; // Assuming 16px = 1rem
      
      console.log(`H1 #${index + 1}:`);
      console.log('  Element:', h1);
      console.log('  Class:', h1.className);
      console.log('  Parent class:', h1.parentElement?.className);
      console.log('  Font size (computed):', fontSize);
      console.log('  Font size in rem:', fontSizeRem.toFixed(2) + 'rem');
      console.log('  CSS Variables:');
      console.log('    --h1-font-size-desktop:', computedStyle.getPropertyValue('--h1-font-size-desktop').trim());
      console.log('    --h1-font-size-mobile:', computedStyle.getPropertyValue('--h1-font-size-mobile').trim());
      console.log('  ---');
    });
    
    // Check CSS variables from root
    const rootStyle = window.getComputedStyle(document.documentElement);
    console.log('Root CSS Variables:');
    console.log('  --h1-font-size-desktop:', rootStyle.getPropertyValue('--h1-font-size-desktop').trim());
    console.log('  --h1-font-size-mobile:', rootStyle.getPropertyValue('--h1-font-size-mobile').trim());
    console.log('  --content-top-spacing-mobile:', rootStyle.getPropertyValue('--content-top-spacing-mobile').trim());
    
    // Check viewport width for media query context
    console.log('Viewport width:', window.innerWidth + 'px');
    console.log('Should use mobile styles:', window.innerWidth <= 768);
    console.log('========================');
  }, 500);
});